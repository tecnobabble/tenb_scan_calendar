...
def io_parse(tio: TenableIO, c: Calendar):
    ...
    for scan in tio.scans.list():
        ...
        # calendar schedule
        if scan['enabled'] and not scan['has_triggers'] \
                and scan['status'] != 'empty':
            scan_details = \
                lib.tenb_common.tenb_response_parse(tio.get('editor/scan/'
                                                    + scan['schedule_uuid']))
            ...
            # End time's are fun
            if scan['type'] == "remote" or scan['type'] == "ps" \
                and scan['status'] == "completed" \
                    or scan['status'] == "aborted" \
                    or scan['status'] == "running":
                scan_time = 0
                history_count = 0
                avg_scan_time = 0
                scan_history = tio.scans.history(
                    scan['schedule_uuid'], limit=10,
                        ...
                        for tag_item in scan_item['default']:
                            for tag_select in scan_item['options']:
                                if tag_item == tag_select['uuid']:
                                    tag_list += tag_select['category_name'] \
                                        + ":" + tag_select['value'] + ""

                    if scan_item['id'] == "text_targets" \
                            and scan_item["default"]:
                ...
                parsed_scan['scan_targets'] = "Targeted Tags: " \
