...
def sc_parse(sc: TenableSC, c: Calendar):
    ...
    for scan in sc.scans.list(['id', 'name', 'policy', 'schedule',
                              'createdTime', 'owner', 'maxScanTime', 'ipList',
                               'assets', 'description'])['manageable']:
        parsed_scan = {}
        if scan['schedule']['enabled'] == "true" \
                and scan['schedule']['start'] != "":
            parsed_scan['raw_timezone'] = scan['schedule']['start']\
                .split('=')[1].split(':')[0]
            parsed_scan['raw_start'] = scan['schedule']['start']\
                .split('=')[1].split(':')[1]
            parsed_scan['starttime_utc'], parsed_scan['starttime_utc_dt'] = \
                lib.tenb_common.return_utc(parsed_scan['raw_timezone'],
                                           parsed_scan['raw_start'])
            ...
            if scan['maxScanTime'] != 'unlimited':
                ...
                parsed_scan['endtime_utc'] = \
                    lib.tenb_common.convert_unix_time(
                        int(parsed_scan['starttime_utc_dt'].timestamp())
                        + endtime)
                ...
                matching_results = []
                scantime = int()
                # loop through scan results and match up with scan name and
                # policy name
    ...
    for scan in lib.tenb_common.tenb_response_parse(
        ...
        if '{schedule[start]}'.format(**scan) != "":
            ...
            parsed_scan['raw_start'] = '{schedule[start]}'.\
                format(**scan).split('=')[1].split(':')[1]
            parsed_scan['starttime_utc'], parsed_scan['starttime_utc_dt'] = \
                lib.tenb_common.return_utc(parsed_scan['raw_timezone'],
                                           parsed_scan['raw_start'])
            ...
            parsed_scan['endtime_utc'] = \
                lib.tenb_common.convert_unix_time(
                    int(parsed_scan['starttime_utc_dt'].timestamp()) + endtime)
            parsed_scan['estimated_run'] = False

            # We have to ask for more data around each agent scan because the
            # tenable.sc api won't let us get it in bulk.
            agent_group = \
                lib.tenb_common.tenb_response_parse(
                    sc.get('agentScan/' + '{id}'.format(**scan)
                           + '?fields=agentGroups'))['agentGroups']

            if len(agent_group) > 0:
                for x in agent_group:
                    group_list += x['name'] + " "
            parsed_scan['scan_targets'] = "Targeted Agent Groups: "\
                + group_list

            # direct agent scans don't have UUIDs for some reason, so we have
            # to generate a consistent one based on some info
            as_seed = '{id}'.format(**scan) + '{createdTime}'.format(**scan)
            rd.seed(as_seed)
            parsed_scan['uuid'] = str(uuid.UUID(int=rd.getrandbits(128),
                                                version=4))
